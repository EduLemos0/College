...
Celulle* newCelulle(Game element) {
    Celulle* new = (Celulle *) malloc(sizeof(Celulle));
    new->game = element;
    new->next = NULL;
    return new; 
}
...
Game removeP() {
    if(top == NULL) { printf("Erro ao remover"); }
    
    Game gameRemoved = top->game;
    Celulle *tmp = top;
    top = top->next;
    tmp->next = NULL;
    free(tmp);
...
void insert(Game game) {
    Celulle* tmp = newCelulle(game);
    tmp->next = top;
    top = tmp;
    tmp = NULL;
}

int flexibleListSize() {
    int counter = 0;
    for(Celulle* i = top; i != NULL; i = i->next, counter++);
    return counter;
}

void showPilhaRec(Celulle* i, int j) {
    if(i != NULL) {
        j--;
        showPilhaRec(i->next, j);
        printf("[%d] ", j);
        game_print(&i->game);
        printf("\n");
    }
...
void pilha(Game fileGamesList[], int numberOfPositions){
    int size = 0;
    char pubIn[10];
    Game game;
